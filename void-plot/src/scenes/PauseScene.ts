...
export class PauseScene extends Scene {
  ...
  public create(): void {
    ...
    const title = this.add.text(this.scale.width / 2, this.scale.height / 2 - 130, "PAUSED", { color: colorToCss(THEME_COLORS.primaryText), fontFamily: THEME_TYPOGRAPHY.fontFamily, fontSize: "28px", fontStyle: "bold" }).setOrigin(0.5);
    ...
    buttons.forEach((button, index) => button.setLayout(this.scale.width / 2 - 120, this.scale.height / 2 - 70 + index * 43, 240, 34));
    ...
    this.events.once("shutdown", () => escapeKey?.off("down", resume));
  }
}
